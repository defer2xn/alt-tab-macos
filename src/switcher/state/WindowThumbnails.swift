...
enum WindowThumbnails {
    ...
    static func refreshAsync(_ windows: [Window], _ source: RefreshCausedBy, windowRemoved: Bool = false, prioritizedIds: Set<CGWindowID>? = nil, force: Bool = false) {
        guard (!windows.isEmpty || windowRemoved) && ScreenRecordingPermission.status == .granted
               ...
               // 后台截图在 macOS 27 上一律抑制（WindowCapturePolicy）：新系统上持续的后台截图会压垮系统截图栈。
        ...
        // titles / appIcons 样式不显示缩略图，只有预览目标（选中窗口及其相邻）需要截图，其余全部跳过，
        // 避免每次唤起都为整个窗口列表白跑一轮截图（WindowCapturePolicy）。
        let previewTargetWids = Appearance.hideThumbnails ? Windows.selectedNeighborhoodIds() : []
        for window in windows {
            if !window.isWindowlessApp, let cgWindowId = window.cgWindowId, cgWindowId != CGWindowID(bitPattern: -1),
               WindowCapturePolicy.shouldCaptureWindow(showsThumbnails: !Appearance.hideThumbnails,
                   isPreviewTarget: previewTargetWids.contains(cgWindowId)),
